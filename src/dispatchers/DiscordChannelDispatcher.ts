...
import { ChannelType as DiscordChannelType, GuildChannelCreateOptions, CategoryChannel, GuildChannel, Snowflake } from "discord.js";
...
type DiscordChannel = Readonly<Pick<GuildChannel, "id" | "name" | "type"> & {
	parent: DiscordChannel | null;
}>;

const CHANNEL_DISPATCHER_NOT_FOUND_ERROR = "The dispatcher channel is not found.";
...
class DiscordChannelDispatcher implements DiscordGuildDispatcher {
	...
	public async create(template: DiscordChannelCreateOptions): Promise<DiscordChannel> {
		...
		if (template.parent) {
			parent = this._guild.channels.cache.find((channel) => {
				return channel.type === DiscordChannelType.GuildCategory && channel.name === template.parent;
			})?.id;
		}
	...
	private async _createChannels(): Promise<void> {
		...
		if (!templates) return;
		const [categories, remaining]: [DiscordChannelCreateOptions[], DiscordChannelCreateOptions[]] = [[], []];
...
export { DiscordChannelCreateOptions, DiscordChannelType, DiscordChannelDispatcherOptions, DiscordChannel, DiscordChannelDispatcher };
